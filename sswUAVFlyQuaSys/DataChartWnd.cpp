...
void CDataChartWnd::FillComboxCtrl()
{
	...
	m_ComboDataExtend.AddString("像片航向重叠度");
	m_ComboDataExtend.AddString("像片旁向重叠度");
	m_ComboDataExtend.AddString("像片原始POS误差");
	m_ComboDataExtend.AddString("像片倾角");
	m_ComboDataExtend.AddString("像片旋角");
	m_ComboDataExtend.AddString("空三困难影像");
}
...
// 	m_pLineSerie = NULL;
	...
	{
	case Q_OVERLAP_Y:
		strLeftAxis = "Overlap_Y";
		break;
	case Q_OVERLAP_X:
		strLeftAxis = "Overlap_X";
		break;
	case Q_POS_ERR:
...
void CDataChartWnd::OnComboDatachartExtend()
{
	...
	CsswUAVFlyQuaSysDoc *pDoc = m_pDoc;
