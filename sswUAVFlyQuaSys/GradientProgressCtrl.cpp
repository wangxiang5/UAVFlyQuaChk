...
// 	bStep = b/(float)nSteps;
	rStep = GetRValue(m_clrEnd);
	gStep = GetGValue(m_clrEnd);
	bStep = GetBValue(m_clrEnd);
	...
	for(int iOnBand = 0; iOnBand < nSteps; iOnBand ++) 
	{
		...
		if(bVertical)
		{
			...
			/*VERIFY(brush.CreateSolidBrush(RGB(r+rStep*iOnBand, g + gStep*iOnBand, b + bStep *iOnBand)));*/  //渐变色
			VERIFY(brush.CreateSolidBrush(RGB(rStep, gStep, bStep)));
...
CMFCGdalProgressCtrl::CMFCGdalProgressCtrl()
{
	m_dPosition = 0.0;
	m_iStepCount = 100;
	m_iCurStep = 0;
...
bool CMFCGdalProgressCtrl::SetPosition(double dPosition)
{
	m_dPosition = min(dPosition, 1.0);
	SetPos(m_dPosition * 100);
...
*/
int CMFCGdalProgressCtrl::StepIt()
{
	m_iCurStep++;
	m_dPosition = min(m_iCurStep*1.0 / m_iStepCount, 1.0);
	SetPos(m_dPosition * 100);
	return true;
}

int STD_API updateProgress(double nstep, const char *pTex, void*pProgressArg)  //gdal进度条回调函数 gdalProcessFunc;
{
// 	CMFCGdalProgressCtrl *pc = (CMFCGdalProgressCtrl*)pProgressArg;
// 	pc->m_dPosition = min(nstep / pc->m_iStepCount, 1.0);
// 	pc->SetPos(pc->m_dPosition * 100);
// 	return 1;

	if (pProgressArg != NULL)
	{
		CMFCGdalProgressCtrl * pProcess = (CMFCGdalProgressCtrl*)pProgressArg;
		pProcess->m_bIsContinue = pProcess->SetPosition(nstep);
