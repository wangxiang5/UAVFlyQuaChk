...
{
	CsswUAVFlyQuaSysEngDoc* pDoc = (CsswUAVFlyQuaSysEngDoc*)p;
	if(pDoc==NULL) return;
...
void WINAPI MultiThreadReOrderStrips(LPVOID p)
{
	...
	CsswUAVFlyQuaSysEngView* pView = (CsswUAVFlyQuaSysEngView*)p;
 	...
 	bool bShow =true;
   	...
   	if(pDoc->m_eFlyQuaChkMethod!=BY_POS) 
   	{
 		...
 		if(!pDoc->m_FlyQuaPrj.BundleAdjustment(param, true))
			{
				MessageBoxEx(NULL,"Adjustment error, control failed!","Error",MB_OK, MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_US));
				pDoc->m_bInRunning = false;
				 return;
		};
		if(pDoc->m_FlyQuaPrj.GetStripNum()==0)
		{
			MessageBoxEx(NULL,"Strip error, control failed!","Error",MB_OK, MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_US));
			return;
		}
...
//	pDoc->m_pView->m_bSaveView2Img = false;
	FunOutPutLogInfo("Preparing report material...");
	float npos = 0;
	pDoc->m_pMain->SendMessage(ID_MSG_WNDPROGRESSCTRL_POS,0,(LPARAM)&npos);
	pDoc->m_pMain->PostMessage(ID_MSG_UPDATE_STATUSLABEL,0,(LPARAM)("Preparing report material..."));
	int nCur = 0;
	for (int i = 5; i>=0; i--)
	{
...
// 	pDoc->m_pMain->m_wndImgAndPt.m_bSaveView2Img=false;
	pDoc->m_pMain->SendMessage(ID_MSG_WNDPROGRESSCTRL_POS,0,(LPARAM)&npos);
	pDoc->m_pMain->PostMessage(ID_MSG_UPDATE_STATUSLABEL,0,(LPARAM)("Ready"));
