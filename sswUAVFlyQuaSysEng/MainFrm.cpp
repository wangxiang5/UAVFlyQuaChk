...
IMPLEMENT_DYNCREATE(CMainFrame, CFrameWndEx)
	...
	//	CreatePaneWnd();
		...
		m_pStatusPaneLabel->SetText("Ready");
	...
	BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
	{
		...
		m_strTitle = "SkyPhoto-QCEng V1.0";
	...
	BOOL CMainFrame::CreateDockingWindows()
	{
		...
		if (!m_wndOutput.Create("Log", this, CRect(0, 0, 100, 100), TRUE, ID_VIEW_OUTPUTWND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_BOTTOM | CBRS_FLOAT_MULTI,AFX_CBRS_REGULAR_TABS,dwNoCloseBarStyle))
		{
			TRACE0("Log window error\n");
			return FALSE; // 未能创建
		}
		if (!m_wndPrjSource.Create("Project", this, CRect(0, 0, 250, 100), TRUE, ID_VIEW_PRJSOURCE_WND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI,AFX_CBRS_REGULAR_TABS,dwNoCloseBarStyle&~AFX_CBRS_RESIZE &~ AFX_CBRS_AUTOHIDE))
		{
			TRACE0("Project window error\n");
			return FALSE; // 未能创建
		}
		if (!m_wndStripImg.Create("Strips", this, CRect(0, 0, 250, 100), TRUE, ID_VIEW_STRIPIMG_WND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_LEFT | CBRS_FLOAT_MULTI,AFX_CBRS_REGULAR_TABS,dwNoCloseBarStyle&~AFX_CBRS_RESIZE &~ AFX_CBRS_AUTOHIDE))
		{
			TRACE0("Strips window error\n");
			return FALSE; // 未能创建
		}
		if (!m_wndImgAndPt.Create("Report Auxiliary Graph", this, CRect(0, 0, 350, 100), TRUE, ID_VIEW_THUMBWND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_RIGHT | CBRS_FLOAT_MULTI,AFX_CBRS_REGULAR_TABS,dwNoCloseBarStyle&~AFX_CBRS_RESIZE &~ AFX_CBRS_AUTOHIDE))
		{
			TRACE0("Report Auxiliary Graph error\n");
			return FALSE; // 未能创建
		}
		if (!m_wndDataChart.Create("Statistical Graph", this, CRect(0, 0, 350, 70), TRUE, ID_VIEW_DATACHART_WND, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_RIGHT | CBRS_FLOAT_MULTI,AFX_CBRS_REGULAR_TABS,dwNoCloseBarStyle&~AFX_CBRS_RESIZE &~ AFX_CBRS_AUTOHIDE))
		{
			TRACE0("Statistical Graph error\n");
			return FALSE; // 未能创建
		}
	...
	BOOL CMainFrame::CreateOutlookBar(CMFCOutlookBar& bar, UINT uiID, CTreeCtrl& tree, int nInitialWidth)
	{
		...
		ASSERT(bNameValid);
		pOutlookBar->AddControl(&tree, "Project", 2, TRUE, dwStyle);
...
// 		else OnMsgOpenPdfReport(0,0);
	...
	LRESULT CMainFrame::OnMsgOpenPdfReport(WPARAM wParam, LPARAM lParam)
	{
		...
		{
			CString strInfo = "There is no report, please run the flight quality control";
		...
		else
		{
			if(eMethd==BY_ADJUSTMENT) 
			{
				...
				m_pDlgRepByAdj->SetWindowText("Report-By Adjustment");
			...
			else if(eMethd==BY_MATCH) 
			{
				...
				m_pDlgRepByMch->SetWindowText("Report-By Matching");
			...
			else if(eMethd==BY_POS) 
			{
				...
				m_pDlgRepByPos->SetWindowText("Report-By POS");
