...
// 	m_strImgFolderPath = "C:\\Users\\Administrator\\Desktop\\wx\\test\\data";
	...
	UpdateData(FALSE);
	m_BrowseImgFolder.m_browserType = eFolder;
...
void CPrjNewDlg::OnEnChangeMfceditbrowseImgfolder()
{
	...
	FunOutPutLogInfo("Image file searching successful.");
...
void CPrjNewDlg::OnBnClickedOk()
{
	...
	{
		MessageBoxEx(NULL,"Please set the project path!","Error",MB_OK, MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_US));
		return;
	}
	if(m_vecAllImgPath.size()==0)
	{
		MessageBoxEx(NULL,"No image exist, please reset!","Error",MB_OK, MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_US));
		return;
	}
	if(m_nCmrIdxSelected==InvalidValue)
	{
		...
		if(cmrInfo.f==0)
		{
			MessageBoxEx(NULL,"No focalLength in image file, please set the camera paras!","Error",MB_OK, MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_US));
			return;
		}
	...
	if(m_nImgPosed == 0)
	{
		if(MessageBoxEx(NULL,"No POS paras, whether or not to continue?","Error",MB_YESNO, MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_US))==IDNO)
		{
			return;
		}
	}
