...
int raw_image_demo_main (char* strImgPath)
{
    HPDF_Doc  pdf;
    HPDF_Font font;
    HPDF_Page page;
    ...
    //HPDF_REAL posY = HPDF_Page_GetHeight (page) - 70;
    ...
    //image = HPDF_LoadRawImageFromFile (pdf, "rawimage\\32_32_rgb.dat",
            //32, 32, HPDF_CS_DEVICE_RGB);
    image = HPDF_LoadRawImageFromFile (pdf,strImgPath,
            1584, 606, HPDF_CS_DEVICE_RGB);
    #endif

    x = (HPDF_Page_GetWidth (page) - HPDF_Image_GetWidth(image)) / 2;
    y = HPDF_Page_GetHeight (page) - 120 - HPDF_Image_GetHeight(image);

    /* Draw image to the canvas. (normal-mode with actual size.)*/
    HPDF_Page_DrawImage (page, image, x, y, 476, 555);
