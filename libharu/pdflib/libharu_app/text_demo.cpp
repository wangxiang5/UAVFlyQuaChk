...
int main_text_demo (int argc, char **argv)
{
    ...
    font_cn = HPDF_GetFont(pdf, "SimSun", "GBK-EUC-H");
    ...
    */
    ...
    HPDF_Page_SetFontAndSize(page, font_cn, 20);
    HPDF_Page_SetTextRenderingMode (page, HPDF_FILL);
    HPDF_Page_BeginText (page);
    //HPDF_Page_TextOut (page, 60, ypos, "ABCabc123");
    HPDF_Page_TextOut (page, 60, ypos, "ÖÐÎÄ×Ö·û");
    HPDF_Page_EndText (page);
    ...
    /*new page to show png pictures*/
    HPDF_Page page_image;      
    HPDF_Destination dst;

    /* add a new page object. */
	page_image = HPDF_AddPage (pdf);

	//HPDF_Page_SetWidth (page_image, 650);
	//HPDF_Page_SetHeight (page_image, 500);

	dst = HPDF_Page_CreateDestination (page_image);
	HPDF_Destination_SetXYZ (dst, 0, HPDF_Page_GetHeight (page_image), 1);
	HPDF_SetOpenAction(pdf, dst);

	HPDF_Page_BeginText (page_image);
	HPDF_Page_SetFontAndSize (page_image, font, 20);
	HPDF_Page_MoveTextPos (page_image, 220, HPDF_Page_GetHeight (page_image) - 70);
	HPDF_Page_ShowText (page_image, "JpegDemo");
	HPDF_Page_EndText (page_image);

	HPDF_Page_SetFontAndSize (page_image, font, 12);    

	draw_image (pdf, page_image, "rgb.jPg", 0, 0, "center");    
    draw_image (pdf, page_image, "VolumeNormalRed.pNg", 5, 5, "center");    
	//draw_image (pdf, "gray.jpg", 340, HPDF_Page_GetHeight (page_image) - 410,
