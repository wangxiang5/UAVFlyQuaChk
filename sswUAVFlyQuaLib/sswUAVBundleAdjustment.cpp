...
// 	}
	...
	if( param.Sift_bDo )
	{
		...
		if( param.pOutputFunc )
		{
			param.pOutputFunc("开始提取特征点，请耐心等待！", -1, NULL, 0,true);
		}
			...
			) != 0)
		{
			if( param.pOutputFunc )
			{
				...
				{
					param.pOutputFunc("特征检测失败，请检查！", -1, NULL, 0,true);
				}
			}
			else
			{
		...
		strInfo.Format("耗时%.3lfs",dt);
		if (param.pOutputFunc)
			param.pOutputFunc("特征提取完成："+strInfo,-1,NULL,0,true);
	}
	...
	{	
		...
		if( param.pOutputFunc )
		{
			param.pOutputFunc("开始特征匹配，请耐心等待！", -1, NULL, 0,true);
		}
			...
			) != 0)
		{
			if( param.pOutputFunc )
			{
				...
				{
					param.pOutputFunc("特征匹配失败，请检查！", -1, NULL, 0,true);
				}
			}
			else
			{
		...
		strInfo.Format("耗时%.3lfs",dt);
		if (param.pOutputFunc)
			param.pOutputFunc("特征匹配完成："+strInfo,-1,NULL,0,true);
		...
		if( param.pOutputFunc )
		{
			param.pOutputFunc("开始串点，请耐心等待！", -1, NULL, 0,true);
		}
			...
			) != 0)
		{
			if( param.pOutputFunc )
			{
				...
				{
					param.pOutputFunc("影像匹配失败，请检查！", -1, NULL, 0,true);
				}
			}
			else
			{
		...
		strInfo.Format("耗时%.3lfs",dt);
		if (param.pOutputFunc)
			param.pOutputFunc("串点完成："+strInfo,-1,NULL,0,true);
	}
	...
	if( param.Freenet_bDo )
	{
		...
		if( param.pOutputFunc )
		{
			param.pOutputFunc("开始自由网平差，请耐心等待！", -1, NULL, 0,true);
		}
		if( Sfm_ImageList(
			m_bIsEnglishVersion,
			...
			) != 0 )
		{
			if( param.pOutputFunc )
			{
				...
				{
					param.pOutputFunc("构建自由网失败，请检查！", -1, NULL, 0,true);
				}
			}
			else
			{
			...
			) != 0)
		{
			if( param.pOutputFunc )
			{
				...
				{
					param.pOutputFunc("自由网整体平差失败，请检查！", -1, NULL, 0,true);
				}
			}
			else
			{
				...
				{
					FunOutPutLogInfo("自由网整体平差失败，请检查！");
				}
			}
			return false;
		}
		...
		strInfo.Format("耗时%.3lfs",dt);
		if (param.pOutputFunc)
			param.pOutputFunc("自由网整体平差完成："+strInfo,-1,NULL,0,true);
	}
	...
	if( param.SBA_bDo )	
	{
		...
		if( param.pOutputFunc )
		{
			param.pOutputFunc("开始整体平差，请耐心等待！", -1, NULL, 0,true);
		}
		if( CSba_zgout(
			m_bIsEnglishVersion,
			(LPSTR)(LPCSTR)imgListFilename,   /// *\imageList.txt
			...
			) != 0)
		{
			if( param.pOutputFunc )
			{
				...
				{
					param.pOutputFunc("整体平差失败，请检查！", -1, NULL, 0,true);
				}
			}
			else
			{
		...
		strInfo.Format("耗时%.3lfs",dt);
		if (param.pOutputFunc)
			param.pOutputFunc("整体平差完成："+strInfo,-1,NULL,0,true);
		...
		if( param.pOutputFunc )
		{
			...
			{
				param.pOutputFunc("正在输出控制点的精度报告！", -1, NULL, 0,true);
			}
		}
		...
		if( param.pOutputFunc )
		{
			...
			{
				param.pOutputFunc("正在输出最终结果！需要几分钟，请耐心等待！", -1, NULL, 0,true);
			}
	...
	if( param.pOutputFunc )
	{
		if(param.Freenet_bDo) param.pOutputFunc("====  影像匹配与平差完成！  ====", -1, NULL, 0,false);
		else if(param.Match_bDo)param.pOutputFunc("====  影像匹配完成！  ====", -1, NULL, 0,false);
	}
	return true;
}
bool CsswBundleAdjustment::LoadData(CString strImgFolder, CString strImgListPath, CString strPosListPath, CString strMatchMatrixPathPath)
{
	...
	{
		FunOutPutLogInfo("影像文件目录有误，空三失败！");
		m_bDataLoad= false;
		return false;
	}
	if(!PathFileExists(strImgListPath)) 
	{
		FunOutPutLogInfo("影像列表文件路径有误，空三失败！");
		m_bDataLoad= false;
		return false;
	}
	if(!PathFileExists(strPosListPath))
	{
		FunOutPutLogInfo("POS列表文件路径有误，空三失败！");
		m_bDataLoad=false;
		return false;
	}
	if(!PathFileExists(strMatchMatrixPathPath))
	{
		FunOutPutLogInfo("匹配列表文件路径有误，空三失败！");
		m_bDataLoad=false;
		return false;
	}
...
bool CsswBundleAdjustment::RunOneKeyMatchAndAdj(CString strMatchAndAdjFolder, sswUAVMatchAndAdjPara& para, bool bRunAdj /* = true */)
{
	...
	if(!m_bDataLoad)
	{
		if(para.pOutputFunc) para.pOutputFunc("数据载入错误，匹配平差执行失败！",-1,NULL,0,true);
		else FunOutPutLogInfo("数据载入错误，匹配平差执行失败！");
		return false;
	}
