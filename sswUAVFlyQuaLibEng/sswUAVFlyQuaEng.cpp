// sswUAVFlyQuaLibEng.cpp : 定义 DLL 应用程序的导出函数。
//
...
static bool XmlNodeJudge(pugi::xml_node& Node)
{
	if(Node==NULL)
	{
		FunOutPutLogInfo("Project file parsing error!");
		return false;
	}
	return true;
}
...
bool CsswUAVFlyQuaPrj::LoadData2Prj(vector<CsswUAVImage> &vecImg,CString strPrjPath, SSWstuSysConfigPara para, bool bWithCmr)
{
	if(vecImg.size()==0)
	{
		FunOutPutLogInfo("Image number is 0, project save failed!");
		return false;
	}
// 	if(vecImg.size()>ImageNumLimite)
// 	{
// 		CString strInfo;
// 		strInfo.Format("The number images is limited(must <%d), project creating failed!",vecImg.size(), ImageNumLimite);
// 		FunOutPutLogInfo(strInfo);
// 		return false;
// 	}
	if(strPrjPath.IsEmpty()) 
	{
		FunOutPutLogInfo("File path error, project save failed!");
		return false;
	}
	ClearPrjData();
	m_bCmrFile = bWithCmr;
...
// 	{
// 		CString strInfo;
// 		strInfo.Format("The number images is limited(must <%d), project creating failed!",m_vecImgInfo.size(), ImageNumLimite);
// 		FunOutPutLogInfo(strInfo);
// 		return false;
// 	}
...
bool CsswUAVFlyQuaPrj::ReadPrjXml()
{
	...
	{
		FunOutPutLogInfo("The project xml file open failed!");
		return false;
	}
										...
										if(ChidLabel=="x0")m_ImgCmrInfo.x0=NodeCmrChid.text().as_double();else
											if(ChidLabel=="y0")m_ImgCmrInfo.y0=NodeCmrChid.text().as_double();else
												if(ChidLabel=="f")
												{
													...
													if(vF.size()!=3){FunOutPutLogInfo("Camera focallength error, project xml file read failed!"); return false;}
													...
													if(ChidLabel=="pixelsize") m_ImgCmrInfo.pixsize=NodeCmrChid.text().as_double();else
														if(ChidLabel=="distortion_paras")
														{
															...
															if(vD.size()!=6){FunOutPutLogInfo("Camera distortion error, project xml file read failed!"); return false;}
...
bool CsswUAVFlyQuaPrj::ReadImpXml()
{
	...
	if(ret.status!=pugi::status_ok)
	{
		FunOutPutLogInfo("The import data xml file open failed!");
		return false;
	}
			...
			if(ChildLabel=="ID") sswImg.m_nImgIdx=Child.text().as_int();else
				if(ChildLabel=="Path")
				{
					...
					if(ChildLabel=="Pos")
					{
						...
						if(vP.size()!=6)
						{
							FunOutPutLogInfo("Image POS parsing error, import xml open failed!");
							return false;
						}
						...
						if(ChildLabel=="StripInfo")
						{
							...
							if(vS.size()!=2)
							{
								FunOutPutLogInfo("Image strip parsing error, import xml open failed!");
								return false;
							}
	...
	{
		FunOutPutLogInfo("Image number error, import xml open failed!");
		ClearPrjData();
		return false;
	}
	if(m_nStripNum>0)
...
bool CsswUAVFlyQuaPrj::SavePrjXml()
{
	if(m_nImgNum==0)
	{
		FunOutPutLogInfo("The image number is 0, project save failed!");
		return false;
	}
	...
	pugi::xml_node Child = ImgNode.append_child("ImageNumber");
...
bool CsswUAVFlyQuaPrj::SaveImgList()
{
	...
	if(pfW==NULL)
	{
		FunOutPutLogInfo("Image list save error, adjustment failed!");
		return false;
	}
	fprintf(pfW,"        \n");
	for (int i = 0; i<m_nImgNum; i++)
...
bool CsswUAVFlyQuaPrj::SavePosList()
{
	...
	if(pfW==NULL)
	{
		FunOutPutLogInfo("POS list save error, adjustment failed!");
		return false;
	}
	for (int i = 0; i<m_nImgNum; i++)
	{
...
bool CsswUAVFlyQuaPrj::SaveMatchMatrix()
{
	...
	{
		FunOutPutLogInfo("Matching list save error, adjustment failed!");
		return false;
	}
...
void CsswUAVFlyQuaPrj::ReOrderStrips(double dLine_a, double dLine_b, double dLine_c,FlyQuaCallBackFun pFun /* = NULL */)
{
	ClearStripInfo();
	ClearReportInfo(BY_POS);
	...
	int nCount = 0;
	if(pFun!=NULL) pFun("Starting strip sorting...",0,NULL,0,true);
	if(pFun!=NULL) pFun("Strip sorting-Coordinate transformation",0,NULL,0,false);
	FunOutPutLogInfo("Strip sorting-Coordinate transformation");
	for (int i = 0; i<m_nImgNum; i++)
	{
	...
	FunOutPutLogInfo("Strip sorting-New coordinate sorting");
	if(pFun!=NULL) pFun("Strip sorting-New coordinate sorting",10,NULL,0,false);
	//2. 按照局部坐标Xs排序-降序
	...
	//4.排列航带
	FunOutPutLogInfo("Strip sorting-Strip calculation");
	if(pFun!=NULL) pFun("Strip sorting-Strip calculation",30,NULL,0,false);
	...
	FunOutPutLogInfo("Strip sorting-Index calculation");
	if(pFun!=NULL) pFun("Strip sorting-Index calculation",60,NULL,0,false);
	//5.航带内排序
	for (int i = 0; i<m_nImgNum-m_nInvalidNum-1; i++)
	{
		for (int j = i+1; j<m_nImgNum-m_nInvalidNum; j++)
	...
	FunOutPutLogInfo("Strip sorting-Reset image index");
	if(pFun!=NULL) pFun("Strip sorting-Reset image index",90,NULL,0,false);
	nCount = 0;
...
fun_end:
	...
	if(pFun!=NULL) pFun("Strip sorting-Process ",100,NULL,0,false);
	if(pFun!=NULL) pFun("Strip sorting-Finished!",-1,NULL,0,false);
	FunOutPutLogInfo("Strip sorting-Finished!");
}
void CsswUAVFlyQuaPrj::ReOrderStripByHand(vector<vector<Point2D>> vvAreaVertex2Select,FlyQuaCallBackFun pFun/* = NULL*/)
{
	//1. 按照绘制的多边形对POS进行航带聚类
	vector<int> vecImgStripIdx(m_nImgNum,InvalidValue);
	if(pFun!=NULL) pFun("Starting strip sorting...",0,NULL,0,true);
	if(pFun!=NULL) pFun("Strip sorting-Strip calculation",0,NULL,0,false);
	FunOutPutLogInfo("Strip sorting-Strip calculation");
#pragma omp parallel for
	for (int i = 0; i<m_nImgNum; i++)
	{
	...
	for (int i = 0; i<vvAreaVertex2Select.size(); i++)
	{
		if(pFun!=NULL) pFun("Strip sorting-Strip calculation",i*40.0/vvAreaVertex2Select.size(),NULL,0,false);
		OGRPolygon *pSelectPolygon = NULL;
		Point2D *pVexterPt = new Point2D[vvAreaVertex2Select[i].size()-1];
		for(int j = 0; j<vvAreaVertex2Select[i].size()-1; j++)
		{
...
#pragma omg parallel for
	...
	FunOutPutLogInfo("Strip sorting-Deleting invalid strips");
	if(pFun!=NULL) pFun("Strip sorting-Deleting invalid strips",45,NULL,0,false);
	//2. 整理顺序
	vector<int>().swap(m_vecImgNumInEachStrip);
	vector<vector<int>>().swap(m_vvImgIdxInEachStrip);
	...
	FunOutPutLogInfo("Strip sorting-Strip index calculation");
	if(pFun!=NULL) pFun("Strip sorting-Strip index calculation",50,NULL,0,false);
	//航带间排序
	...
	if(m_nStripNum>0)
	{
		// 		dMaxX = dMinX = m_vecImgInfo[m_vvImgIdxInEachStrip[0][0]].m_ImgPosInfo.Xs;
		// 		dMaxY = dMinY = m_vecImgInfo[m_vvImgIdxInEachStrip[0][0]].m_ImgPosInfo.Ys;
		dMaxX = dMaxY = -1e10;
		dMinX = dMinY = 1e10;
		for (int i = 0 ; i<m_vecImgNumInEachStrip[0]; i++)
		{
			dMinX=min(dMinX,m_vecImgInfo[m_vvImgIdxInEachStrip[0][i]].m_ImgPosInfo.Xs);
			dMaxX=max(dMaxX,m_vecImgInfo[m_vvImgIdxInEachStrip[0][i]].m_ImgPosInfo.Xs);
			dMinY=min(dMinY,m_vecImgInfo[m_vvImgIdxInEachStrip[0][i]].m_ImgPosInfo.Ys);
			dMaxY=max(dMaxY,m_vecImgInfo[m_vvImgIdxInEachStrip[0][i]].m_ImgPosInfo.Ys);
		}
	}
	SSWOVERLAPDIRECTION eDirStrip = (dMaxY-dMinY)>(dMaxX-dMinX)?DIRECTION_Y:DIRECTION_X;
	...
	for (int i = 0; i<m_nStripNum; i++)
	{
		...
		{
			int nImgIdx = m_vvImgIdxInEachStrip[i][j];
			vecdStripMeanX[nStripIdxNow]+=m_vecImgInfo[nImgIdx].m_ImgPosInfo.Xs;
			vecdStripMeanY[nStripIdxNow]+=m_vecImgInfo[nImgIdx].m_ImgPosInfo.Ys;
		}
		vecdStripMeanX[nStripIdxNow]/=(m_vecImgNumInEachStrip[i]);
		vecdStripMeanY[nStripIdxNow]/=(m_vecImgNumInEachStrip[i]);
	}
	if(pFun!=NULL) pFun("Strip sorting-Strip index calculation",65,NULL,0,false);
	FunOutPutLogInfo("Strip sorting-Strip index calculation-65");
	...
	else if(eDirStrip==DIRECTION_X)
	{
		for (int i = 0; i<m_nStripNum-1; i++)
		{
			for (int j = i+1; j<m_nStripNum; j++)
			{
				if(vecdStripMeanY[i]<vecdStripMeanY[j])
				{
					swap(vecdStripMeanY[i],vecdStripMeanY[j]);
					swap(m_vecStripIdx[i],m_vecStripIdx[j]);
					swap(m_vecImgNumInEachStrip[i],m_vecImgNumInEachStrip[j]);
					swap(m_vvImgIdxInEachStrip[i],m_vvImgIdxInEachStrip[j]);
				}
	...
	if(pFun!=NULL) pFun("Strip sorting-Strip index calculation",70,NULL,0,false);
	FunOutPutLogInfo("Strip sorting-Strip index calculation-70");
#pragma omp parallel for
	for (int i = 0; i<m_nStripNum; i++)
	{
	...
	// 	{
	// 		if(!m_vecImgInfo[i].m_bValid) continue;
	// 		for(int j = 0; j<m_nStripNum; j++)
	// 		{
	// 			if(vecImgStripIdx[i]==m_vecStripIdx[j])
	// 			{
	// 				m_vecImgInfo[i].m_nStripIdx=j;
	// 				vecImgStripIdx[i]=j;
	// 			}
	// 		}
	// 	}
	FunOutPutLogInfo("Strip sorting-Image index calculation");
	if(pFun!=NULL) pFun("Strip sorting-Image index calculation",75,NULL,0,false);
	//航带内排序
	int nCount = 0;
	if(eDirStrip==DIRECTION_Y)
	{
		for (int i = 0; i<m_vecImgNumInEachStrip.size(); i++)
		{
			if(pFun!=NULL) pFun("Strip sorting-Image index calculation",75+20.0*i/m_vecImgNumInEachStrip.size(),NULL,0,false);
...
#pragma omp parallel for
	...
	vector<double>().swap(vecdStripMeanX);
	vector<double>().swap(vecdStripMeanY);
	if(pFun!=NULL) pFun("Strip sorting-Process",100,NULL,0,false);
	if(pFun!=NULL) pFun("Strip sorting-Finished!",-1,NULL,0,false);
	FunOutPutLogInfo("Strip sorting-Finished!");
	//	ReadImgList();
...
{
	FunOutPutLogInfo("Camera distortion correction and image resampling...");
	clock_t t1 = clock();
	int nValidNum = m_nImgNum - m_nInvalidNum;
	int nCount = 0;
	pFun("Distortion correction and image resampling...",0,NULL,0,true);
	CRITICAL_SECTION cs;
...
#pragma omp parallel for num_threads(omp_get_num_procs()-1)
	for (int i = 0; i<m_nImgNum; i++)
	{
		...
		if(pFun!=NULL)
		{
			...
			{
				strInfo.Format("Distortion correction and image resampling...");
	...
	sprintf(strlog,"distortion correction and image resampling finished: Image number %d, time %.3lfs",m_nImgNum-m_nInvalidNum,m_dTimeSecsCmrDistortion);
	if(pFun!=NULL)
	{
		CString strInfo="Distortion correction and image resampling...";
...
bool CsswUAVFlyQuaPrj::BundleAdjustment(sswUAVMatchAndAdjPara para, bool bChkExist /* = true */)
{
	...
	if(para.Freenet_bDo)
	{
		if(!PathFileExists(m_strBundleAdjRepPath))
		{
			...
			if(para.pOutputFunc)
			{
				para.pOutputFunc("Image matching and adjustment...",-1,NULL,0,true);
				//			para.pOutputFunc("开始缩略图匹配平差...",-1,NULL,0,true);
			}
			else  FunOutPutLogInfo("Image matching and adjustment...");
			...
			sprintf(strlog,"Matching and adjustment finished, time %.3lfs",m_dTimeSecsBundleAdjustment);
	...
	else if(para.Match_bDo)
	{
		if(!PathFileExists(m_strMatchPointPath))
		{
			...
			t1 = clock();
			if(para.pOutputFunc)
				para.pOutputFunc("Image matching...",-1,NULL,0,true);
			else  FunOutPutLogInfo("Image matching...");
			CreateDirectory(m_strMatchAndAdjFolder,NULL);
			CsswBundleAdjustment UavImgMatch;
			...
			t2 = clock();
			m_dTimeSecsBundleAdjustment=(double)(t2-t1)/CLOCKS_PER_SEC;
			sprintf(strlog,"Image matching finished, time %.3lfs",m_dTimeSecsBundleAdjustment);
			if(para.pOutputFunc)  para.pOutputFunc(strlog,0,NULL,0,true);
			else FunOutPutLogInfo(strlog);
		}
		return true;
...
void CsswUAVFlyQuaPrj::CalStripOverlaps(double Z, SSWFLYQUAMETHOD eMethod,FlyQuaCallBackFun pFun /* = NULL */)
{
	...
	for (int i = 0; i<m_nStripNum; i++)
	{
		...
		if(pFun!=NULL)
		{
			strInfo.Format("Overlap calculation...%-3.1lf%%",40+nCount*20.0/m_nStripNum);
	...
	for (int i = 0; i<m_nStripNum; i++)
	{
		...
		{
			CString strInfo;
			strInfo.Format("Side Overlap calculation...%-3.1lf%%",60+nCount*20.0/m_nStripNum);
...
bool CsswUAVFlyQuaPrj::ReadAdjRep(FlyQuaCallBackFun pFun/* =NULL */)
{
	...
	{
		FunOutPutLogInfo("Adjustment report open error, data read filed!");
		return false;
	}
	char line[MAX_SIZE_FILE_LINE];
	const char* tmp = txt.GetFileHead();
	if(!ReadLine(tmp,line,MAX_SIZE_FILE_LINE))
	{
		FunOutPutLogInfo("Adjustment report 1st line read error, data read filed!");
		return false;
	}
	tmp+=strlen(line)+2;
	int nImgNum, nPtNum;
	...
	{
		FunOutPutLogInfo("Adjustment report is incomplete, data read filed!");
		return false;
	}
	char strlog[MAX_SIZE_FILE_LINE];
	SSWstuPosInfo* pAdjPosInfo = new SSWstuPosInfo[nImgNum];
	if(pFun!=NULL) pFun("Reading adjustment report...",10,NULL,0,true);
	for (int i =0; i<nImgNum; i++)
	{
		SSWstuPosInfo sswAdjPos;
		if(!ReadLine(tmp,line,MAX_SIZE_FILE_LINE))
		{	
			sprintf(strlog,"Adjustment result, image %d read failed!",i);
			FunOutPutLogInfo(strlog);
			return false;
		}
		tmp+=strlen(line)+2;
		...
		{
			sprintf(strlog,"Adjustment result, image %d parse failed!",i);
			FunOutPutLogInfo(strlog);
			return false;
		}
		...
		if(!ReadLine(tmp,line,MAX_SIZE_FILE_LINE))
		{
			sprintf(strlog,"Adjustment result, image %d exterior elements read failed!",i);
			FunOutPutLogInfo(strlog);
			return false;
		}
		tmp+=strlen(line)+2;
		...
		{
			sprintf(strlog,"Adjustment result, image %d exterior elements parse failed!",i);
			FunOutPutLogInfo(strlog);
			return false;
		}
		if(!ReadLine(tmp,line,MAX_SIZE_FILE_LINE))
	...
	for (int i = 0; i<nPtNum; i++)
	{
		...
		if(pFun!=NULL)
		{
			...
			{
				strInfo = "Reading adjustment report......";
...
#pragma omp parallel for
	...
	CString strInfo="Reading adjustment report......";
	...
	sprintf(strlog,"Reading adjustment report finished: Image number %d, Point number %d, Time %.3lfs",nImgNum,nPtNum,m_dTimeSecsTimeReadAdjRep);
	if(pFun!=NULL)
	{
...
bool CsswUAVFlyQuaPrj::ReadMatchPoint(FlyQuaCallBackFun pFun /* = NULL */)
{
	...
	{
		FunOutPutLogInfo("Matching result file open error, data read failed!");
		return false;
	}
	if(pFun)
	{
		pFun("Reading matching result...",0,NULL,0,true);
		pFun("Reading matching result...",5,NULL,0,false);
	}
	const char* tmp = txt.GetFileHead();
	...
	if(!ReadLine(tmp,line,MAX_SIZE_FILE_LINE))
	{
		FunOutPutLogInfo("Matching result 1st line read error, data read failed!");
		return false;
	}
	tmp+=strlen(line)+2;
	if(2!=sscanf(line,"%d %d",&nImgNum,&nPtNum))
	{
		FunOutPutLogInfo("Matching result 1st line parsing error, data read failed!");
		return false;
	}
	...
	for (int i = 0; i<nPtNum*2; i++)
	{
		...
		*(pAdjPtsLine+i) = line;
		if(pFun)
		{
			...
			{
				CString strInfo;
				strInfo.Format("Reading matching result...",5+5*(i+1)/nPtNum);
	...
	for (int i = 0; i<nPtNum; i++)
	{
		...
		if(pFun!=NULL)
		{
			...
			{
				strInfo.Format("Reading matching result...",10+85.0*(nCount+1)/nPtNum);
				pFun(strInfo,20+75.0*(nCount+1)/nPtNum,NULL,0,false);
			}
		}
		// 		LeaveCriticalSection(&cs);
	...
	txt.CloseFile();
	FunArrangeAdjPts(m_vecImgInfo,m_vecAdjPts, m_vecPtNumInImgs);
	CString strInfo="Reading matching result...";
	if(pFun!=NULL)	pFun(strInfo,97,NULL,0,false);

	if(pAdjPtsLine)delete[]pAdjPtsLine;

	strInfo="Reading matching result...";
	if(pFun!=NULL) pFun(strInfo,100,NULL,0,false);
	clock_t t2 = clock();
	double dt = (double)(t2-t1)/CLOCKS_PER_SEC;
	m_dTimeSecsTimeReadAdjRep = dt;
	sprintf(strlog,"Reading matching result finished: Image number %d, Point number %d, Time %.3lfs",nImgNum,nPtNum,m_dTimeSecsTimeReadAdjRep);
	if(pFun!=NULL)
	{
		pFun(strlog,100,NULL,0,true);	
	}
...
{ 
	...
	if(eMethod==BY_MATCH) 
	{
		...
		{
			if(!pFun) FunOutPutLogInfo("Matching result read error, quality control failed!");
			else pFun("Matching result read error, quality control failed!",0,NULL,0,true);
			return false;
		}
	}
	else if(eMethod==BY_ADJUSTMENT)
	{
		if(!ReadAdjRep(pFun))
		{
			if(!pFun) FunOutPutLogInfo("Adjustment result read error, quality control failed!");
			else pFun("Adjustment result read error, quality control failed!",0,NULL,0,true);
			return false;
		}
	}
	if(pFun!=NULL) pFun("Quality control calculation：Image Pitch",0,NULL,0,true);
	CalImagePitch();
	if(pFun!=NULL) pFun("Quality control calculation：Image Yaw",10,NULL,0,true);
	CalImageYaw(Z,eMethod/*BY_MATCH*/);
	if(pFun!=NULL) pFun("Quality control calculation：Flight Alt Differ",20,NULL,0,true);
	CalImageFlyHeiErr(eMethod);
	if(pFun!=NULL) pFun("Quality control calculation：Flight Direction Deviation",30,NULL,0,true);
	CalImageFlyDisErr(eMethod);
	if(pFun!=NULL) pFun("Quality control calculation：Overlap & Side-Overlap",40,NULL,0,true);
	...
	if(pFun!=NULL) pFun("Quality control calculation：Image POS Residual",80,NULL,0,true);
	CalImagePosErr(eMethod);
	if(pFun!=NULL) pFun("Quality control calculation：Paras Calculation",90,NULL,0,true);
	AnalysisFlyQuaChkParas(m_nMatchPointNumThresold);
	if(pFun!=NULL) pFun("Quality control calculation：Missing Zone Analysis",95,NULL,0,true);
	CheckMissArea(Z,eMethod);
	if(pFun!=NULL) pFun("Quality control calculation：Missing Zone Analysis",100,NULL,0,false);
	CalStripCqPaneErr(eMethod);
	clock_t t2 = clock();
	double dt = (double)(t2-t1)/CLOCKS_PER_SEC;
	m_dTimeSecsRunFlyQuaChk = dt;
	char strlog[MAX_SIZE_FILE_LINE];
	sprintf(strlog,"Quality control calculation finished, time %.3lfs",m_dTimeSecsRunFlyQuaChk);
...
{
	FunOutPutLogInfo("Starting to write control report...");
	clock_t t1 = clock();
	...
	if(!pdf.InitPdf())
	{
		FunOutPutLogInfo("PDF initialization failed!");
		return false;
	}
	int nImgIdx =1; 
	char strImgLabelInfo[MAX_SIZE_FILE_LINE]; 
	...
	if(!pdf.AddPage()) return false;
	pdf.SetLineSpace(50);
	pdf.SetPdfTitle("UAV Flight Quality Control Report",0,0.6,0);
	CString strLogon = FunGetFileFolder(FunGetThisExePath())+"\\logo.dll";
	int nw = 31, nh = 31;
	int nx = pdf.GetPdfPageWidth()-pdf.GetLineSpace();
	int ny = pdf.GetPdfPageHeight()-pdf.GetTitleGap()-pdf.GetLineGap();
	pdf.AddImage(strLogon,1.0,SSW_NONE,nx,ny,nw,nh); 
	sprintf(strLine,"Report Generation Time: %s",FunGetSysTimeStr());
	pdf.AddText(strLine,SSW_SONG,1.2,SSW_LEFT,0,false,0,0,0,0,0,0,true);pdf.AddEmptyLine(-1.2);
	//pdf.AddText("@2016 UAV Flight Quality Check",SSW_SONG,1.2,SSW_RIGHT,0,false,0.3,0.3,0.3,0,0,0,true);
	pdf.AddEmptyLine(5);
	...
	/* 第一部分 质检报告概述                                                 */
	/************************************************************************/
	pdf.AddText("Summarize",SSW_HEI,2.5,SSW_CENTER);
	pdf.AddEmptyLine(2);
	memset(strLine,'-',MAX_SIZE_FILE_LINE*3);
	pdf.AddText(strLine,SSW_SONG,1,SSW_LEFT,0,true,0.7,0.7,0.7);
	pdf.AddEmptyLine(-2);
	sprintf(strLine,"Control Method——By Adjustment");
	...
	pdf.AddEmptyLine(3);
	/**************测区影像数量 ***************/
	pdf.AddText("1. Image Overview",SSW_HEI,2,SSW_LEFT);
	pdf.AddEmptyLine(2);
	nListCols = 4; nListRows = 2;
	vecListInfo.resize(nListCols*nListRows);
	vecListInfo[0*nListCols+0].Format("%s","Image Num");
	vecListInfo[0*nListCols+1].Format("%s","Valid Num");
	vecListInfo[0*nListCols+2].Format("%s","Invalid Num");
	vecListInfo[0*nListCols+3].Format("%s","Strip Num");
	vecListInfo[1*nListCols+0].Format("%d",m_nImgNum);
	vecListInfo[1*nListCols+1].Format("%d",m_nImgNum-m_nInvalidNum);
	vecListInfo[1*nListCols+2].Format("%d",m_nInvalidNum);
	vecListInfo[1*nListCols+3].Format("%d",m_nStripNum);
	...
	vector<CString>().swap(vecListInfo);
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("Image overview finished!");

	/**************数据完整性检查 ***************/
	pdf.AddText("2. Missing Zone",SSW_HEI,2,SSW_LEFT);
	pdf.AddEmptyLine(2);
	pdf.AddImage(strCqViewImgPath);
	pdf.AddEmptyLine(0.5);
	sprintf(strImgLabelInfo,"Figure %d. Strip map(The red point is invalid images, blue is valid, and red polygon is missing zone)",nImgIdx);
	pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
	nImgIdx++;
	pdf.AddEmptyLine(1);
	sprintf(strLine,"Like figure %d, missing zone number is %d, and it's area proportion is %3.2lf%%.",1,m_vvMissAreaImgIdx.size(),m_dMissAreaRatio*100);
	pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1,0,0);
// 	if(m_vvMissAreaImgIdx.size()>0)
// 	{
// 		pdf.AddEmptyLine(1.5);
...
// 	}
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("Missing zone finished!");

	/***********************影像重叠度信息*************************/
	pdf.AddText("3. Forward-Overlap and Side-Overlap Information",SSW_HEI,2,SSW_LEFT);
	pdf.AddEmptyLine(2);
	CString strOverlapImg = FunGetFileFolder(strCqViewImgPath)+"\\OverlapImg_ByBundle.jpg";
	pdf.AddImage(strOverlapImg,1,SSW_CENTER,0,0,0,0,true);
	//pdf.AddImage(FunGetFileFolder(strViewImgPath)+"\\OverlapNote.jpg",1.0,SSW_NONE,0,350,40);
	pdf.AddEmptyLine(0.5);
	sprintf(strImgLabelInfo,"Figure %d. Overlap map",nImgIdx);
	pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
	nImgIdx++; 
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("Forward-Overlap & Side-Overlap information finished!");

	/***********************空三困难影像*************************/
	pdf.AddText("4. Difficulty Adjustment Image(DAI)",SSW_HEI,2,SSW_LEFT);
	pdf.AddEmptyLine(2);
	pdf.AddImage(strMatchMatchErrImgPath,2);
	pdf.AddEmptyLine(0.5);
	sprintf(strImgLabelInfo,"Figure %d. DAI map(The blue point is DAI, green one is qualified)",nImgIdx);
	pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
	nImgIdx++; 
	pdf.AddEmptyLine(1);
	sprintf(strLine,"Like figure %d, DAI（Point number<%d) number is %d.",3,m_nMatchPointNumThresold,m_vecImgIdxLowMatchPointNum.size());
	pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1,0,0);
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("DAI finished!");

	/***********************质检结论*************************/
	pdf.AddText("5. Conclusion",SSW_HEI,2,SSW_LEFT);
	pdf.AddEmptyLine(2);
	nListCols=3;nListRows=6;
	vecListInfo.resize(nListCols*nListRows);
	vecListInfo[0*nListCols+0].Format("Item");
	vecListInfo[0*nListCols+1].Format("Result");
	vecListInfo[0*nListCols+2].Format("Remark");

	vecListInfo[1*nListCols+0].Format("Camera");
	if(!m_bCmrFile)
	{
		vecListInfo[1*nListCols+1].Format("Null");
		vecListInfo[1*nListCols+2].Format("Camera paras are missing.");
	}
	else
	{
		if(m_dMaxPaneDifErr>m_FlyQuaParaSet.fMaxErrStationPane&&m_dRmsPaneDifErr>m_FlyQuaParaSet.fRmsErrStationPane)
		{
			vecListInfo[1*nListCols+1].Format("Disqualification");
			vecListInfo[1*nListCols+2].Format("Self calibration is suggested.");
		}
		else
		{
			vecListInfo[1*nListCols+1].Format("Qualification");
			vecListInfo[1*nListCols+2].Format("Null");
		}
	}
	vecListInfo[2*nListCols+0].Format("Missing Zone");
	if (m_vvMissAreaImgIdx.size()==0)
	{
		vecListInfo[2*nListCols+1].Format("Qualification");
		vecListInfo[2*nListCols+2].Format("Null");
	}
	else
	{
		if(m_dMissAreaRatio>m_FlyQuaParaSet.fMissAreaRatio/100)
		{
			vecListInfo[2*nListCols+1].Format("Disqualification");
			vecListInfo[2*nListCols+2].Format("Missing zone proportion is %3.2lf%%, adding flight is suggested.",m_dMissAreaRatio*100);
		}else
		{
			vecListInfo[2*nListCols+1].Format("Qualification");
			vecListInfo[2*nListCols+2].Format("Missing zone proportion is %3.2lf%%, acceptable.",m_dMissAreaRatio*100);
		}
	}
	vecListInfo[3*nListCols+0].Format("Overlap");
	vecListInfo[3*nListCols+1].Format("Figure 2");
	vecListInfo[3*nListCols+2].Format("Qualification with over 5 times overlap number of main zone.");
	vecListInfo[4*nListCols+0].Format("DAI");
	if (m_vecImgIdxLowMatchPointNum.size()==0)
	{
		vecListInfo[4*nListCols+1].Format("0");
		vecListInfo[4*nListCols+2].Format("Null");
	}
	else
	{
		vecListInfo[4*nListCols+1].Format("%d ",m_vecImgIdxLowMatchPointNum.size());
		vecListInfo[4*nListCols+2].Format("Details in difficulty adjustment image list.");
	}
	vecListInfo[5*nListCols+0].Format("Image Attitude Angle");
	float fAngleRatio = (m_vecImgIdxHeiYaw.size()+m_vecImgIdxHeiPitch.size())*1.0/(m_nImgNum-m_nInvalidNum);
	if (fAngleRatio<m_FlyQuaParaSet.fAngleErrRatio/100)
	{
		vecListInfo[5*nListCols+1].Format("Qualification");
		vecListInfo[5*nListCols+2].Format("Null");
	}
	else
	{
		vecListInfo[5*nListCols+1].Format("Disqualification");
		vecListInfo[5*nListCols+2].Format("Angle error image proportion is %3.2lf%%, a new flight is suggested.",fAngleRatio*100);
	}
	cprintf("Result...");
	pdf.AddList(nListCols,nListRows,vecListInfo);
	cprintf("...Over\n");
	vector<CString>().swap(vecListInfo);
	FunOutPutLogInfo("Conclusion finished!");
	pdf.AddEmptyLine(2);
	pdf.AddText("More remarks: ",SSW_HEI,1.5,SSW_LEFT,0,false,1);
	pdf.AddEmptyLine(1);
	sprintf(strLine,"%s%s","1) Causes of DAI: Weak texture, repetition texture(like water, forest, snow and desert); ",
		"Poor image quality, like shadow shade, overexposure and fuzzy image.");
	pdf.AddText(strLine, SSW_SONG,1,SSW_LEFT,0,false);
	pdf.AddEmptyLine(1);
	sprintf(strLine,"%s","2) Unqualified attitude angle will leads to bad adjustment result, and makes DLG processing more difficult.");
	pdf.AddText(strLine,SSW_SONG,1,SSW_LEFT,0,false);
	pdf.AddEmptyLine(5);
	...
	/* 第二部分 质检报告详述                                                 */
	/************************************************************************/
	pdf.AddText("Expand",SSW_HEI,2.5,SSW_CENTER);
	pdf.AddEmptyLine(3);
	/***********************质检参数阈值*************************/
	nListCols = 3; nListRows = 10;
	vecListInfo.resize(nListCols*nListRows);
	vecListInfo[0*nListCols+0]="Item";
	vecListInfo[0*nListCols+1]="Threshold description";
	vecListInfo[0*nListCols+2]="Value";
	vecListInfo[1*nListCols+0]="Camera";
	vecListInfo[1*nListCols+1]="The max distance of camera POS to station pane after adjustment(m)";
	vecListInfo[1*nListCols+2].Format("%.2f",m_FlyQuaParaSet.fMaxErrStationPane);
	vecListInfo[2*nListCols+0]="Camera";
	vecListInfo[2*nListCols+1]="The rms of distance of camera POS to station pane after adjustment(m)";
	vecListInfo[2*nListCols+2].Format("%.2f",m_FlyQuaParaSet.fRmsErrStationPane);
	vecListInfo[3*nListCols+0]="Forward-Overlap";
	vecListInfo[3*nListCols+1]="The qualified min value";
	vecListInfo[3*nListCols+2].Format("%.2f%%",m_FlyQuaParaSet.fMinOverlapY);
	vecListInfo[4*nListCols+0]="Side-Overlap";
	vecListInfo[4*nListCols+1]="The qualified min value";
	vecListInfo[4*nListCols+2].Format("%.2f%%",m_FlyQuaParaSet.fMinOverlapX);
	vecListInfo[5*nListCols+0]="Pitch";
	vecListInfo[5*nListCols+1]="The qualified max value";
	vecListInfo[5*nListCols+2].Format("%.2f",m_FlyQuaParaSet.fMaxPitch);
	vecListInfo[6*nListCols+0]="Yaw";
	vecListInfo[6*nListCols+1]="The qualified max value";
	vecListInfo[6*nListCols+2].Format("%.2f",m_FlyQuaParaSet.fMaxYaw);
	vecListInfo[7*nListCols+0]="DAI";
	vecListInfo[7*nListCols+1]="The qualified min point number";
	vecListInfo[7*nListCols+2].Format("%d",m_nMatchPointNumThresold);
	vecListInfo[8*nListCols+0]="Missing Zone";
	vecListInfo[8*nListCols+1]="The qualified max proportion of missing zone area";
	vecListInfo[8*nListCols+2].Format("%.2f%%",m_FlyQuaParaSet.fMissAreaRatio);
	vecListInfo[9*nListCols+0]="Attitude Angle";
	vecListInfo[9*nListCols+1]="The qualified max proportion of error attitude image";
	vecListInfo[9*nListCols+2].Format("%.2f%%",m_FlyQuaParaSet.fAngleErrRatio);
	pdf.AddText("1. Quality Control Threshold",SSW_HEI,2,SSW_LEFT);
	pdf.AddEmptyLine(2);
	cprintf("========List info in report========\n");
	cprintf("Threshold...");
	pdf.AddList(nListCols,nListRows,vecListInfo);
	...
	vector<CString>().swap(vecListInfo);
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("Quality control threshold finished!");

	/***********************相机内参数分析*************************/
	pdf.AddText("2. Camera Intrinsic Parameter Analysis",SSW_HEI,2,SSW_LEFT);
	pdf.AddEmptyLine(2);
	if(m_bCmrFile)
	{
		pdf.AddText("Original distortion paras",SSW_SONG,1.5,SSW_LEFT);
		pdf.AddEmptyLine(1);
		nListCols = 6; nListRows = 2;
		vecListInfo.resize(nListCols*nListRows);
		vecListInfo[0*nListCols+0].Format("a");
...
// 		pdf.AddEmptyLine(1.5);
		...
		if(m_dMaxPaneDifErr>m_FlyQuaParaSet.fMaxErrStationPane&&m_dRmsPaneDifErr>m_FlyQuaParaSet.fRmsErrStationPane)
		{
			sprintf(strLine,"In the adjustment result, the deviation of camera POS to station pane, max value is %.2lfm(greater than threshold %.2fm), rms value is %.2lfm(greater than threshold %.2fm).",m_dMaxPaneDifErr,m_FlyQuaParaSet.fMaxErrStationPane,m_dRmsPaneDifErr,m_FlyQuaParaSet.fRmsErrStationPane);
			pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,0);
			pdf.AddEmptyLine(1.5);
			pdf.AddText("Unqualified camera paras!",SSW_SONG,1.5,SSW_LEFT,0,false,1);
		}
		else
		{
			if(m_dMaxPaneDifErr>m_FlyQuaParaSet.fMaxErrStationPane)
			{
				sprintf(strLine,"In the adjustment result, the deviation of camera POS to station pane, max value is %.2lfm(greater than threshold %.2fm), rms value is %.2lfm(less than threshold %.2fm).",m_dMaxPaneDifErr,m_FlyQuaParaSet.fMaxErrStationPane,m_dRmsPaneDifErr,m_FlyQuaParaSet.fRmsErrStationPane);
			}else 
			if(m_dRmsPaneDifErr>m_FlyQuaParaSet.fRmsErrStationPane)
			{
				sprintf(strLine,"In the adjustment result, the deviation of camera POS to station pane, max value is %.2lfm(less than threshold %.2fm), rms value is %.2lfm(greater than threshold %.2fm).",m_dMaxPaneDifErr,m_FlyQuaParaSet.fMaxErrStationPane,m_dRmsPaneDifErr,m_FlyQuaParaSet.fRmsErrStationPane);
			}else
			{
				sprintf(strLine,"In the adjustment result, the deviation of camera POS to station pane, max value is %.2lfm(less than threshold %.2fm), rms value is %.2lfm(less than threshold %.2fm).",m_dMaxPaneDifErr,m_FlyQuaParaSet.fMaxErrStationPane,m_dRmsPaneDifErr,m_FlyQuaParaSet.fRmsErrStationPane);
			}
			pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,0);
			pdf.AddEmptyLine(1.5);
			pdf.AddText("Qualified camera paras!",SSW_SONG,1.5,SSW_LEFT,0,false,1);
		}
		pdf.AddEmptyLine(1.5);
		float *pX = new float[m_nInAdjedNum];
		float *pY = new float[m_nInAdjedNum];
		...
		pdf.AddChart(pY,pX,m_vecDisImgAdjPos2PosPane.size(),0,0,1,0,0,1,0,0,"Deviation of POS to Station Pane","Image Index","POS Deviation Value-m");
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. Deviation of POS to Station Pane",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
		delete[]pX;
		delete[]pY;
	...
	{
		pdf.AddText("No camera paras!",SSW_SONG,1.5,SSW_LEFT,0,false,1);
	}
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("Camera paras finished!");

	/***********************POS精度*************************/	
	pdf.AddText("3. Original POS Analysis",SSW_HEI,2,SSW_LEFT);
	pdf.AddEmptyLine(2);
	sprintf(strLine,"The number of image in adjustment is %d, and the differ of original POS and adjustment result(Residual-RES), RMS = %.2lfm, MEAN = %.2lfm.",
		m_nInAdjedNum,m_dRmsPosErr,m_dMeanPosErr);
	pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false);
	pdf.AddEmptyLine(1);
	//	sprintf(strLine,"不合格影像数%d张（Err > 3*RMS+均值 为不合格）",m_vecImgIdxHeiPosErr.size());
	sprintf(strLine,"The number of Image with POS RES over 3 times of RMS（RES>3*RMS+MEAN）is %d.",m_vecImgIdxHeiPosErr.size());
	pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1);
	pdf.AddEmptyLine(1.5);
	if(m_vecImgIdxHeiPosErr.size()>0)
	{	
		nListCols = 6; nListRows = m_vecImgIdxHeiPosErr.size()+1;
		vecListInfo.resize(nListRows*nListCols);
		vecListInfo[0*nListCols+0].Format("Image Id");
		vecListInfo[0*nListCols+1].Format("Image Name");
		vecListInfo[0*nListCols+2].Format("RES_X-m");
		vecListInfo[0*nListCols+3].Format("RES_Y-m");
		vecListInfo[0*nListCols+4].Format("RES_Z-m");
		vecListInfo[0*nListCols+5].Format("RES_XYZ-m");
		for (int i = 0; i<nListRows-1; i++)
		{
			vecListInfo[(i+1)*nListCols+0].Format("%d",m_vecImgIdxHeiPosErr[i]);
			vecListInfo[(i+1)*nListCols+1].Format("%s",m_vecImgInfo[m_vecImgIdxHeiPosErr[i]].m_strImgName);
		...
		pdf.AddImage(strPosErrViewImgPath,2);
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. The distribution of images with POS over limit(The blue point is image over limit, green one is the other type)",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
		pdf.AddEmptyLine(3);
	}
	...
	pdf.AddChart(pY,pX,m_nInAdjedNum,0,0,1,0,0,1,0,0,"POS Residual-XYZ Chart","Image Index","POS RES Value-m");
	pdf.AddEmptyLine(0.5);
	sprintf(strImgLabelInfo,"Figure %d. Graph of image POS residual-XYZ",nImgIdx);
	pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
	nImgIdx++;
	...
	delete[]pY;
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("POS Residual finished!");


	/***********************航飞漏洞*************************/
	pdf.AddText("4. Mission Zone",SSW_HEI,2);
	...
	if(m_vvMissAreaImgIdx.size()>0)
	{
		...
		sprintf(strLine,"Image list of missing zone");
		pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1,0,0);
		pdf.AddEmptyLine(1.5);
		nListCols=4;nListRows=nMissImgNum+1;
		vecListInfo.resize(nListCols*nListRows);
		vecListInfo[0*nListCols+0].Format("Zone Id");
		vecListInfo[0*nListCols+1].Format("Image Id");
		vecListInfo[0*nListCols+2].Format("Image Name");
		vecListInfo[0*nListCols+3].Format("Strip Info");
		int nCount = 1;
		for (int i = 0; i<m_vvMissAreaImgIdx.size(); i++)
		{
			for (int j = 0; j<m_vvMissAreaImgIdx[i].size(); j++)
	...
	{
		pdf.AddEmptyLine(2);
		sprintf(strLine,"No obvious missing zone!");
		pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1,0,0);
	}
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("Missing zone finished");
	...
	/***********************重叠度*************************/
	sprintf(strLine,"5. Forward-Overlap and Side-Overlap");
	pdf.AddText(strLine,SSW_HEI,2,SSW_LEFT);
	pdf.AddEmptyLine(2);
	double dMaxOverlapX, dMinOverlapX,dMaxOverlapY, dMinOverlapY;
	double dMeanOverlapX=0,dMeanOverlapY=0;
	...
	if(nCount>0)
	{
		dMeanOverlapX/=nCount;
		dMeanOverlapY/=nCount;
		sprintf(strLine,"Each image's overlap is calculated as the max overlap value between it and its adjacent images.");
		pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT);
		pdf.AddEmptyLine(1);
		sprintf(strLine,"Forward-Overlap: the max value is %3.2lf%%, min value is %3.2lf%%, and mean value is %3.2lf%%;",dMaxOverlapY*100,dMinOverlapY*100,dMeanOverlapY*100);
		pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT);
		pdf.AddEmptyLine(1);
		sprintf(strLine,"Side-Overlap: the max value is %3.2lf%%, min value is %3.2lf%%, and mean value is %3.2lf%%.",dMaxOverlapX*100,dMinOverlapX*100,dMeanOverlapX*100);					
		pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT);
		pdf.AddEmptyLine(2);
	}
	sprintf(strLine,"The number of image with unqualified Forward-Overlap(< %.2lf%%) is %d.",m_FlyQuaParaSet.fMinOverlapY,m_vecImgIdxLowOverlapInsStrips.size());
	pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1,0,0);
	if(m_vecImgIdxLowOverlapInsStrips.size()>0)
	{
		pdf.AddEmptyLine(1);
		...
		vecListInfo[0*nListCols+0].Format("Image Id");
		vecListInfo[0*nListCols+1].Format("Image Name");
		vecListInfo[0*nListCols+2].Format("Strip Info");
		vecListInfo[0*nListCols+3].Format("Overlap Value");
		for (int i = 0; i<nListRows-1; i++)
		{
			int nImgIdx = m_vecImgIdxLowOverlapInsStrips[i];
			double dOverlap=
		...
		pdf.AddImage(strOverlapYErrViewImgPath,2);
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. The distribution of images with unqualified Forward-Overlap(The blue point is unqualified image, green one is qualified)",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
		pdf.AddEmptyLine(3);
	}
	FunOutPutLogInfo("Forward-Overlap finished");
	pdf.AddEmptyLine(2);
	sprintf(strLine,"The number of image with unqualified Side-Overlap(< %.2lf%%) is %d.",m_FlyQuaParaSet.fMinOverlapX,m_vecImgIdxLowOverlapBetStrips.size());
	pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1,0,0);
	char strlog[MAX_SIZE_FILE_LINE];
	if(m_vecImgIdxLowOverlapBetStrips.size()>0)
	{
		...
		pdf.AddImage(strOverlapXErrViewImgPath,2);
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. The distribution of images with unqualified Side-Overlap(The blue point is unqualified image, green one is qualified)",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
		pdf.AddEmptyLine(3);
	}
	...
	{
		FunSortByX(nCount,pX,pOverlapY,pOverlapX);
		pdf.AddChart(pOverlapY,pX,nCount,0,0,1,0,0,1.0,0,0,"Forward-Overlap","Image Index","Value(%)");
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. Graph of image Forward-Overlap",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
		pdf.AddEmptyLine(1.5);
		pdf.AddChart(pOverlapX,pX,m_nImgNum-m_nInvalidNum,0,0,1,0,0,1.0,0,0,"Side-Overlap","Image Index","Value(%)");
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. Graph of image Side-Overlap",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
	}
	delete[]pOverlapX;	
	...
	delete[]pX;
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("Side-Overlap finished!");

	/***********************空三困难影像*************************/
	pdf.AddText("6. Difficulty Adjustment Image",SSW_HEI,2);
	pdf.AddEmptyLine(2);
	if(m_vecImgIdxLowMatchPointNum.size()>0)
	{
		nListCols = 3; nListRows = m_vecImgIdxLowMatchPointNum.size()+1;
		...
		vecListInfo[0*nListCols+0].Format("Image Id");
		vecListInfo[0*nListCols+1].Format("Image Name");
		vecListInfo[0*nListCols+2].Format("Point Num");
		for (int i = 0; i<m_vecImgIdxLowMatchPointNum.size(); i++)
		{
			vecListInfo[(i+1)*nListCols+0].Format("%d",m_vecImgIdxLowMatchPointNum[i]);
			vecListInfo[(i+1)*nListCols+1].Format("%s",m_vecImgInfo[m_vecImgIdxLowMatchPointNum[i]].m_strImgName);
	...
	{
		pdf.AddText("No difficulty adjustment image!",SSW_SONG,1.5,SSW_LEFT,0,true,1);
	}
	pdf.AddEmptyLine(3);
	FunOutPutLogInfo("Difficulty adjustment image finished!");
	/***********************影像姿态角评价*************************/
	pdf.AddText("7. Image Attitude Angle",SSW_HEI,2);
	pdf.AddEmptyLine(2);
	sprintf(strLine,"The number of image with unqualified pitch(> %.2lfdeg) is %d.",m_FlyQuaParaSet.fMaxPitch,m_vecImgIdxHeiPitch.size());
	pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1);
	pdf.AddEmptyLine(1);
	if(m_vecImgIdxHeiPitch.size()>0)
	{	
		...
		vecListInfo[0*nListCols+0].Format("Image Id");
		vecListInfo[0*nListCols+1].Format("Image Name");
		vecListInfo[0*nListCols+2].Format("Pitch Value(deg)");
		for (int i = 0; i<nListRows-1; i++)
		{
			int nImgIdx = m_vecImgIdxHeiPitch[i];
			vecListInfo[(i+1)*nListCols+0].Format("%d",nImgIdx);
		...
		pdf.AddImage(strPitchErrViewImgPath,2);
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. The distribution of image with unqualified pitch(The blue point is unqualified, green one is qualified)",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
		pdf.AddEmptyLine(3);
	}	
	...
	if(nCount>0)
	{
		pdf.AddChart(pPitch,pX,nCount,0,0,1,0,0,1,0,0,"Image Pitch","Image Index","Value(deg)");
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. Graph of image pitch",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
		pdf.AddEmptyLine(3);
	}
	FunOutPutLogInfo("Image pitch finished!");
	sprintf(strLine,"The number of image with unqualified yaw(> %.2lfdeg) is %d.",m_FlyQuaParaSet.fMaxYaw,m_vecImgIdxHeiYaw.size());
	pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1);
	pdf.AddEmptyLine(1);
	if(m_vecImgIdxHeiYaw.size()>0)
	{
		...
		vecListInfo[0*nListCols+0].Format("Image Id");
		vecListInfo[0*nListCols+1].Format("Image Name");
		vecListInfo[0*nListCols+2].Format("Yaw Value(deg)");
		for (int i = 0; i<nListRows-1; i++)
		{
			int nImgIdx = m_vecImgIdxHeiYaw[i];
			vecListInfo[(i+1)*nListCols+0].Format("%d",nImgIdx);
		...
		pdf.AddImage(strYawViewImgPath,2);
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. The distribution of image with unqualified yaw(The blue point is unqualified, green one is qualified)",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
		pdf.AddEmptyLine(3);
	}
	...
	if(nCount>0)
	{
		pdf.AddChart(pYaw,pX,m_nImgNum-m_nInvalidNum,0,0,1,0,0,1,0,0,"Image Yaw","Image Index","Value(deg)");
		pdf.AddEmptyLine(0.5);
		sprintf(strImgLabelInfo,"Figure %d. Graph of image yaw",nImgIdx);
		pdf.AddText(strImgLabelInfo,SSW_HEI,1,SSW_CENTER);
		nImgIdx++;
	}
	delete[]pYaw;
	delete[]pPitch;
	delete[]pX;
	FunOutPutLogInfo("Image yaw finished!");
	pdf.AddEmptyLine(3);
	sprintf(strLine,"The number of invalid image is %d.",m_nInvalidNum);
	pdf.AddText(strLine,SSW_SONG,1.5,SSW_LEFT,0,false,1);
	pdf.AddEmptyLine(1);
	if(m_nInvalidNum>0) 
	{
	...
	FunOutPutLogInfo("Invalid image finished!");
	pdf.AddEmptyLine(3);

	/***********************其他质检信息*************************/
	pdf.AddText("8. Processing Time Statistics",SSW_HEI,2);
	pdf.AddEmptyLine(2);
	clock_t t2 = clock();
	double dt = (double)(t2-t1)/CLOCKS_PER_SEC;
	m_dTimeSecsWriteChkRep=dt;
	...
	nListCols = 2; nListRows = 7;
	vecListInfo.resize(nListCols*nListRows);
	vecListInfo[0*nListCols+0].Format("Process Part");
	vecListInfo[0*nListCols+1].Format("Time-s");
	vecListInfo[1*nListCols+0].Format("The Whole Process");
	vecListInfo[1*nListCols+1].Format("%.3lf",m_dTimeSecsWholeProcess);
	vecListInfo[2*nListCols+0].Format("Distortion Correction");
	vecListInfo[2*nListCols+1].Format("%.3lf",m_dTimeSecsCmrDistortion);
	vecListInfo[3*nListCols+0].Format("Adjustment");
	vecListInfo[3*nListCols+1].Format("%.3lf",m_dTimeSecsBundleAdjustment);
	vecListInfo[4*nListCols+0].Format("Parsing Adjustment Report");
	vecListInfo[4*nListCols+1].Format("%.3lf",m_dTimeSecsTimeReadAdjRep);
	vecListInfo[5*nListCols+0].Format("Quality Control Calculation");
	vecListInfo[5*nListCols+1].Format("%.3lf",m_dTimeSecsRunFlyQuaChk);
	vecListInfo[6*nListCols+0].Format("Saving Control Report");
	vecListInfo[6*nListCols+1].Format("%.3lf",m_dTimeSecsWriteChkRep);
	cprintf("TimeInfo...");
	bool b = pdf.AddList(nListCols,nListRows,vecListInfo);
	cprintf("...Over\n");
	vector<CString>().swap(vecListInfo);
	pdf.AddEmptyLine(1);
	FunOutPutLogInfo("Report finished!");
	CString strPath = m_strFlyQuaChkRepPath;
