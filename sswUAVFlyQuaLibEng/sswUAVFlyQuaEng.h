...
public:
	...
	//重排航带
	void ReOrderStrips(double dLine_a, double dLine_b, double dLine_c,FlyQuaCallBackFun pFun = NULL);
	void ReOrderStripByHand(vector<vector<Point2D>> vvAreaVertex2Select,FlyQuaCallBackFun pFun = NULL);
	//相机畸变纠正及影像重采样生成缩略图
	...
	{
		m_strFlyQuaChkRepPath = FunGetFileFolder(m_strPrjXmlPath)+"\\"+FunGetFileName(m_strPrjXmlPath,false)+"_Flight_Quality_Control_Report.pdf";
